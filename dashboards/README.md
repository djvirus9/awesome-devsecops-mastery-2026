...
`python3 scripts/validate_dashboard.py` runs the fixture with a Linux-container Docker daemon. It pulls pinned official Python, Prometheus, Grafana and Grafana Image Renderer images, then creates four task-named containers on a temporary internal network. Prometheus and the renderer are not published; Grafana gets a random host-loopback port. The containers share the exporter's network namespace, so the existing loopback-only exporter does not need a public bind. No host network, Docker-socket mount, host sensor, production dataset, or existing Grafana installation is used.
